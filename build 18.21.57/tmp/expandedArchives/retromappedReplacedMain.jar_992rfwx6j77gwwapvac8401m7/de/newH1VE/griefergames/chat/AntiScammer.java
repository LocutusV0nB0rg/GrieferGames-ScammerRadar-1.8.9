...
public class AntiScammer extends Chat {
    ...
    public void updateTabList() {
        Collection<NetworkPlayerInfo> playerMap = LabyModCore.getMinecraft().getConnection().func_175106_d();
    }
    ...
    public void modifyTabList() {
        ...
        try {
            for (NetworkPlayerInfo player : playerMap) {
                boolean found = false;
                IChatComponent componentName = player.func_178854_k();
                String playerItem = componentName.func_150260_c();

                if (playerItem != null) {

                    String[] playerComponent = playerItem.split("\\u2503");
                    String playerName = playerComponent[1].trim();
                    for (Scammer scammer : onlineScammerList) {
                        if (playerName.equals(scammer.name)) {
                            found = true;
                            if (!playerItem.startsWith("[SCAMMER]")) {
                                ...
                                IChatComponent newplayer = befsign.func_150257_a(sign).func_150257_a(afsign).func_150257_a(componentName);
                                player.func_178859_a(newplayer);
                            }
                        }
                    }
                    ...
                    if (playerItem.startsWith("[SCAMMER]") && found == false) {
                        IChatComponent newname = componentName.func_150253_a().get(componentName.func_150253_a().size() - 1);
                        player.func_178859_a(newname);
                    }
                }
            }
